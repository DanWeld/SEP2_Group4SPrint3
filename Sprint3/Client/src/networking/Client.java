...
public class Client
{
  ...
  /**
   * Generic method to call the server with a method name and parameters
   * 
   ...
   */
  public <T> T call(String methodName, Object params, Class<T> returnType) throws Exception {
    if (!connected) {
    ...
    try {
      ...
      String paramsJson = JsonParser.toJson(params);
      ...
      if (response.startsWith("ERROR:")) {
        throw new Exception(response.substring(7));
      }
      
      // Parse the response
      return (T) JsonParser.jsonToObject(response, returnType);
    } catch (IOException e) {
